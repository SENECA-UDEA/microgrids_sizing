...
def create_objects(generators, batteries, forecast_df, demand_df, instance_data, my_data):
    ...
    for k in generators:
      if k['tec'] == 'S':
        ...
        obj_aux.Solargeneration( forecast_df['t_ambt'], gt, instance_data["G_stc"], my_data["sol_deg"])
        obj_aux.Solarcost()
      elif k['tec'] == 'W':
        obj_aux = Eolic(*k.values())
        obj_aux.Windgeneration(forecast_df['Wt'],instance_data["h2"],instance_data["coef_hel"], my_data["wind_deg"] )
        obj_aux.Windcost()
      elif k['tec'] == 'D':

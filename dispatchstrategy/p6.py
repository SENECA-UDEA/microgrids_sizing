...
for ppp in range(1, 37):
    for kkk in range(1,37):
        iii = 1
        jjj = 1
        ...
        bat_cost_run = 1 - (ppp-1)*0.02
        jadd_name7 = 'bat'+str(bat_cost_run)+'%cost'
        ren_cost_run = 1 - (kkk-1)*0.02
        jadd_name8 = 'ren'+str(ren_cost_run)+'%cost'          
  
        # file paths local
        demand_filepath = "../data/"+place+"/demand_"+place+".csv"
        forecast_filepath = "../data/"+place+"/forecast_"+place+".csv"
        units_filepath = "../data/"+place+"/parameters_"+place+"_sind.json"
        ...
        if (sol_feasible.results != None):
            ...
            name_escj =  str(jjj) + ' ' + str(jadd_name1) + ' ' + str(jadd_name2)+ ' ' + str(jadd_name3)+ ' ' + str(jadd_name4)+ ' ' + str(jadd_name5)+ ' ' + str(jadd_name6)+ ' ' + str(jadd_name7)+ ' ' + str(jadd_name8)
            name_esc = 'esc_'   + name_esci + ' MODEL- ' + name_escj
            rows_df_time.append([iii,jjj,ppp,kkk,name_esc, place, iteraciones_run, amax, tlpsp_run, nse_run, aux_instance_data['splus_cost'],
                                bat_cost_run, ren_cost_run,
            ...
            df_time = pd.DataFrame(rows_df_time, columns=["N problem","N algortihm","ppp","kkk", "Name", "City", "Iterations", "Area","Tlpsp",
                                                          "NSE","S+_cost","bat cost", "ren cost",
                                                          "S-_cost", "fuel_cost","Len_demand","Demand percent",
